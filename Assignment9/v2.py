...
def viterbi(positions, genotypes, p_ref, error_rate=1 / 1000):
    ...
    for sample, genotype_list in genotypes.items():
        ...
        for i in range(n - 1, -1, -1):
            if current_state == 0:  # Inbred state
                if start is None:
                    start = positions[i]
                if i == 0 or traceback[current_state, i] != 0:  # Transition out of inbred
                    inbred_regions.append((positions[i], start))
                    start = None
            else:  # Outbred state
                if start is not None:  # End of an inbred region
                    inbred_regions.append((positions[i + 1], start))
                    start = None
