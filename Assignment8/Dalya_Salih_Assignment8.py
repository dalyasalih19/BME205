...
preprocessor = ColumnTransformer(
    transformers=[
        ('num', RobustScaler(), numerical_cols),          # More robust scaling
        ('cat', OneHotEncoder(drop='first'), categorical_cols)  # Drop first for efficiency
    ]
)
...
X_train, X_val, y_train, y_val = train_test_split(
    X_preprocessed, y, test_size=0.1, random_state=42  # Smaller test set for more training data
)
