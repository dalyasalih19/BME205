...
def normalize_expression(data):
    ...
    This ensures all expression values are relative to the total expression for each sample.
...
def read_and_normalize(directory):
    ...
    # Loop through each file, read the gene expression data, and normalize it
    for file in all_files:
        with open(file, mode='r') as infile:
            ...
            for row in reader:  # Process each gene in the file
                gene_expression.append(float(row[1]))  # Extract and convert gene expression values to float
    ...
    # Convert the list of lists to a numpy array for easier processing in later steps
...
def calculate_log2_fold_change(mean_control, mean_treatment):
    ...
    The fold change measures how much a gene is up or downregulated in the treatment group relative to control.
...
def mann_whitney_u_manual(control_data, treatment_data):
    ...
    This test compares the medians of the control and treatment groups to see if they are significantly different.
    ...
    """
    num_genes = control_data.shape[1]  # Get the number of genes
    p_values = []

    # Perform the Mann-Whitney U test for each gene
    for gene_idx in range(num_genes):
        # Get the gene expression data for both control and treatment groups for this specific gene
        ...
        ranks = np.argsort(np.argsort(combined_data)) + 1  # Assign ranks starting from 1
        
        # Split the ranks back into control and treatment groups
        ranks_control = ranks[:len(control_gene_data)]
        ranks_treatment = ranks[len(control_gene_data):]
        
        # Compute U-statistics for both control and treatment groups
        ...
        U = min(U1, U2)  # Use the smaller U value
        
        # Calculate mean and standard deviation for U based on sample sizes
        n1 = len(control_gene_data)
        n2 = len(treatment_gene_data)
        mean_U = n1 * n2 / 2
        std_U = np.sqrt(n1 * n2 * (n1 + n2 + 1) / 12)
        
        # Calculate the z-score for the Mann-Whitney U test
        z = (U - mean_U) / std_U
        
        # Use NormalDist to compute the two-sided p-value based on the z-score
        normal_dist = statistics.NormalDist(mu=0, sigma=1)
        p_value = 2 * (1 - normal_dist.cdf(abs(z)))  # Two-tailed test
...
def main():
    ...
    # Compute mean and median normalized expression for each gene in both groups
    ...
    # Collect all data into a list of tuples for sorting by p-value
    gene_data = []
    ...
    # Sort gene data by p-value (smallest p-value first)
    gene_data.sort(key=lambda x: x[6])  # Sort by p-value (index 6 in the tuple)

    # Output sorted results to a file with the appropriate header
